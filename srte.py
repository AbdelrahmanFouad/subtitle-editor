...
def translate_batch(client, model_id, block_texts):
    config = types.GenerateContentConfig(
        system_instruction=(
            ...
            "Maintain line breaks. For each block, output ONLY the Arabic lines. "
            "Precede each translation with its index in brackets, like [1], [2], etc."
        ),
        temperature=0.3,
    )
...
if st.button("Translate Entire File (in batches)"):
    ...
    while current_progress < total_pending:
        ...
        try:
            ...
            progress_bar.progress(current_progress / total_pending)
            time.sleep(10)

        except Exception as e:
            err = str(e).lower()
            if any(x in err for x in ["quota", "429", "resourceexhausted"]):
                st.warning(f"Key #{key_idx + 1} limited. Rotating account...")
                time.sleep(2)
    ...
    if current_progress >= total_pending:
